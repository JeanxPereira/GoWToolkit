                           ...
                           const std::string& packName)
{
    ...
    if (!fs.is_open()) {
        ONYX_LOGF_WARN("[LodPackIndex] NOT FOUND: %s", path.string().c_str());
        return 0;
    }
    ...
    ONYX_LOGF_INFO("[LodPackIndex]   [%d] Opening: %s", packIdx, label.c_str());
    ...
    ONYX_LOGF_INFO("[LodPackIndex]     segments=%d  lod_entries=%d", segCount, lodCount);

    if (segCount <= 0 || lodCount <= 0 ||
        segCount > 100000 || lodCount > 5000000) {
        ONYX_LOGF_ERR("[LodPackIndex] Implausible header values — skipping pack");
        return 0;
    }
    ...
    for (int s = 0; s < segCount; ++s) {
        ...
        if (s < 10) {
            ONYX_LOGF_INFO("[LodPackIndex]     seg[%d] base_offset=0x%llX",
    ...
    for (int j = 0; j < lodCount; ++j) {
        ...
        if (!fs) {
            ONYX_LOGF_WARN("[LodPackIndex] Unexpected EOF at lod[%d]", j);
            break;
        }

        if (segIdx < 0 || segIdx >= segCount) {
            ONYX_LOGF_WARN("[LodPackIndex] lod[%d]: invalid segIdx=%d — skipping", j, segIdx);
            continue;
        }
        ...
        if (j < 10 || (j < 20 && !isDup)) {
            ONYX_LOGF_INFO("[LodPackIndex]     lod[%d] key=0x%016llX  seg=%d  off=0x%llX  size=%d  %s",
    ...
    ONYX_LOGF_INFO("[LodPackIndex]   Added %d unique entries from %s", added, label.c_str());
    return added;
}
                                ...
                                const std::filesystem::path& gameRoot)
{
    ...
    if (!txt.is_open()) {
        ONYX_LOGF_ERR("[LodPackIndex] Cannot open lodpacks.txt: %s",
    ...
    ONYX_LOGF_INFO("[LodPackIndex] lodpacks.txt: %zu pack(s) listed", packNames.size());
    ...
    ONYX_LOGF_INFO("[LodPackIndex] Total unique lod entries indexed: %d", total);
    return total;
}
...
int LodPackIndex::LoadFromGameRoot(const std::filesystem::path& gameRoot) {
    ...
    if (!std::filesystem::exists(pcLeDir, ec)) {
        ONYX_LOGF_WARN("[LodPackIndex] pc_le directory not found: %s", pcLeDir.string().c_str());
        return 0;
    }
    ...
    ONYX_LOGF_INFO("[LodPackIndex] %zu lodpack(s) found in %s", packs.size(),
                             ...
                             std::vector<uint8_t>& outData) const
{
    ...
    if (!fs.is_open()) {
        ONYX_LOGF_ERR("[LodPackIndex] Cannot open pack[%d]: %s",
    ...
    if (!fs) {
        ONYX_LOGF_ERR("[LodPackIndex] Seek failed to offset 0x%llX in pack[%d]",
    ...
    if (!fs) {
        ONYX_LOGF_ERR("[LodPackIndex] Read failed: requested %d bytes at 0x%llX",

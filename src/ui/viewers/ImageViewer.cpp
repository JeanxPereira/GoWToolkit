...
ImageViewer::ImageViewer(const std::string& name, std::unique_ptr<TextureData> texture)
...
void ImageViewer::Draw() {
    ...
    if (ImGui::SmallButton("Zoom+")) m_zoom = std::min(m_zoom * 1.5f, 16.0f);
    ImGui::SameLine();
    if (ImGui::SmallButton("Zoom-")) m_zoom = std::max(m_zoom / 1.5f, 0.125f);
    ImGui::SameLine();
    if (ImGui::SmallButton("1:1")) m_zoom = 1.0f;
    ImGui::SameLine();
    ...
    ImGui::BeginChild("##texview", avail, false, ImGuiWindowFlags_HorizontalScrollbar);
    ...
    ImGui::Image((void*)(intptr_t)m_glTexture, ImVec2(imgW, imgH), ImVec2(0, 0), ImVec2(1, 1));
    
    // Scroll zoom
    if (ImGui::IsItemHovered()) {
        float wheel = ImGui::GetIO().MouseWheel;
        if (wheel > 0) m_zoom = std::min(m_zoom * 1.2f, 16.0f);
        if (wheel < 0) m_zoom = std::max(m_zoom / 1.2f, 0.125f);
    }

...
SoundPlayer::SoundPlayer(const std::string& name, std::unique_ptr<GOW2SoundParser::SoundBankData> bankData)
    : m_name(name)
...
SoundPlayer::SoundPlayer(const std::string& name, std::vector<int16_t> pcmData, uint32_t sampleRate, uint32_t channels)
    : m_name(name)
    , m_audio(std::make_unique<AudioDevice>())
...
void SoundPlayer::SelectSound(int index) {
    if (index < 0 || index >= (int)m_bankData->sounds.size()) return;
    ...
    if (snd.adpcmOffset + snd.adpcmSize > m_bankData->bankStreamData.size()) {
        m_decodedPcm.clear();
        LOG_ERR("[SoundPlayer] ADPCM data out of bounds for '%s': offset=0x%X size=0x%X streamSize=0x%zX",
                snd.name.c_str(), snd.adpcmOffset, snd.adpcmSize, m_bankData->bankStreamData.size());
        return;
    }

    m_decodedPcm = AdpcmDecoder::Decode(
        m_bankData->bankStreamData.data() + snd.adpcmOffset,
        snd.adpcmSize);

    LOG_INFO("[SoundPlayer] Decoded '%s': %zu PCM samples", snd.name.c_str(), m_decodedPcm.size());
}

void SoundPlayer::Play() {
    if (!m_audio || !m_audio->initialized) return;
    if (m_decodedPcm.empty()) return;
...
void SoundPlayer::DrawToolbar() {
    ...
    if (m_singleSoundMode) {
    ...
    } else if (m_selectedSound >= 0 && m_bankData && m_selectedSound < (int)m_bankData->sounds.size()) {
        ...
        if (!m_decodedPcm.empty()) {
            ...
            float total = (float)m_decodedPcm.size() / (float)m_bankData->defaultSampleRate;
            ImGui::TextDisabled("%.1f / %.1fs", seconds, total);
        }
    } else {
...
void SoundPlayer::DrawWaveform() {
    ...
    // Draw waveform
    ImGui::PlotLines("##waveform_plot", displayData.data(), displayWidth,
                     0, nullptr, -1.0f, 1.0f, ImVec2(avail.x, avail.y - 20));

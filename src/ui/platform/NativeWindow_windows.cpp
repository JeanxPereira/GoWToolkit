...
// ── WM_SETCURSOR: repassa cursores ImGui → Win32 ──────────────────────────
...
// ── WndProc borderless — 1:1 com ImHex ───────────────────────────────────
// Forward declaration — implementado em main.cpp via glfwSetWindowUserPointer
...
static LRESULT CALLBACK borderlessWindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    switch (uMsg) {
        ...
        case WM_NCHITTEST: {
            ...
            // Tamanho da borda (1:1 ImHex — sem DPI scale porque GLFW já escala)
            const POINT border {
...
void setup(GLFWwindow* window, bool borderless) {
    ...
    if (borderless) {
        ...
        // modo com decoração nativa — sem WndProc custom
    }
}
...
void beginFrame(GLFWwindow* window, float titleBarHeight) {
    ...
    // Sem isto o resize NÃO FUNCIONA em modo borderless.
    {

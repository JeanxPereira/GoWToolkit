...
void SceneRenderer::Build(const SceneData& scene) {
    ...
    for (const auto& part : scene.meshParts) {
        ...
        // DIAGNOSTIC — log first batch's jointMap and first 5 vertex boneIndices/positions
        if (m_batches.size() == 1) {
...
void SceneRenderer::RenderBatches(const std::vector<RenderBatch*>& batches,
    ...
    for (auto* batch : batches) {
        ...
        // Joint palette — build a per-batch remapped palette so that
